...
type Adapter struct {
	...
	port   int
	server *grpc.Server
	eventEmitter.UnimplementedEventEmitterServer
}
...
func (a Adapter) Run() {
	...
	a.server = grpcServer
	eventEmitter.RegisterEventEmitterServer(grpcServer, a)
	if config.GetEnv() == "development" {
		reflection.Register(grpcServer)
	}

	log.Printf("starting 1eventEmitter service on port %d ...", a.port)
