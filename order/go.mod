module github.com/ayuved/microservices/order

go 1.22.3

require go.opentelemetry.io/contrib/instrumentation/google.golang.org/grpc/otelgrpc v0.52.0

require (
	github.com/ayuved/microservices-proto/golang/payment v1.0.1 // indirect
	github.com/uptrace/opentelemetry-go-extra/otelsql v0.3.0 // indirect
)

require (
	filippo.io/edwards25519 v1.1.0 // indirect
	github.com/ayuved/microservices-proto/golang/order v1.0.2
	github.com/go-logr/logr v1.4.2 // indirect
	github.com/go-logr/stdr v1.2.2 // indirect
	github.com/go-sql-driver/mysql v1.8.1 // indirect
	github.com/jinzhu/inflection v1.0.0 // indirect
	github.com/jinzhu/now v1.1.5 // indirect
	github.com/sirupsen/logrus v1.9.3
	github.com/uptrace/opentelemetry-go-extra/otelgorm v0.3.0
	go.opentelemetry.io/otel v1.27.0 // indirect
	go.opentelemetry.io/otel/exporters/jaeger v1.17.0
	go.opentelemetry.io/otel/metric v1.27.0 // indirect
	go.opentelemetry.io/otel/sdk v1.27.0
	go.opentelemetry.io/otel/trace v1.27.0 // indirect
	golang.org/x/net v0.26.0 // indirect
	golang.org/x/sys v0.21.0 // indirect
	golang.org/x/text v0.16.0 // indirect
	google.golang.org/genproto/googleapis/rpc v0.0.0-20240604185151-ef581f913117 // indirect
	google.golang.org/grpc v1.64.0
	google.golang.org/protobuf v1.34.1 // indirect
	gorm.io/driver/mysql v1.5.6
	gorm.io/gorm v1.25.10
)
